...
const OrderHistory_Individual: React.FC = () => {
  const IoChevronDown = wrapIcon(IoIcons.IoChevronDown);
    ...
    <div className="min-h-screen bg-[#EDF4ED] font-sans">
      ...
      <main className="px-8 py-6 max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center flex-wrap gap-6">
          ...
          <div className="text-center">
            ...
            <p className="text-sm mt-3">
              <span className="font-semibold">occasional user</span>
              <br />
              earn <span className="text-green-700 font-bold">
                300 points
              </span>{" "}
              to level up
            </p>
        ...
        <div className="space-y-6">
          {orders.map((order) => (
            <Link to={`/track/${order.id}`} key={order.id} className="block">
              <div className="bg-white rounded-xl p-4 shadow border flex flex-col md:flex-row items-start md:items-center gap-4 hover:shadow-md transition">
                <img
                  src={order.image}
                  alt={order.title}
...
export default OrderHistory_Individual;
